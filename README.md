...
- 先做确定性分类和事故聚合，再把压缩后的证据交给 AI；AI 只能为已存在的 issue 润色只读排查建议，不能修改严重度、计数、玩家、位置、证据、分类或 issue 集合。
...
主报告模型使用最小 JSON 协议，只返回 `category/tag/incident_index` 绑定的 `suggested_action`。独立的候选 issue reviewer 可以在高置信度下删除明确误报；其余事实始终来自确定性初稿。最终 prompt 的 timeline、fallback、聊天统计、异常样本和抽样记录统一经过 URL、邮箱、UUID、IP 与长 token 脱敏，避免清洗文本从旁路字段重新泄漏。
